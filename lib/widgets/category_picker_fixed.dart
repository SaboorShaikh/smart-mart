...
class _CategoryPickerState extends State<CategoryPicker> {
  ...
  void _filterCategories(String query) {
    ...
    debugPrint(
        'CategoryPicker: Current filtered categories count: ${_filteredCategories.length}');
    setState(() {
      if (query.isEmpty) {
        _filteredCategories = widget.categories;
        debugPrint(
            'CategoryPicker: Showing all ${widget.categories.length} categories');
      } else {
        ...
        debugPrint(
            'CategoryPicker: Found ${_filteredCategories.length} matching categories');
        debugPrint(
            'CategoryPicker: Filtered categories: ${_filteredCategories.take(5).join(", ")}${_filteredCategories.length > 5 ? "..." : ""}');
      }
    });
  }
        ...
        children: [
          ...
          Expanded(
            child: _filteredCategories.isEmpty
                ...
                : ListView.builder(
                    key: ValueKey(_filteredCategories
                        .length), // Force rebuild when list changes
                    padding: const EdgeInsets.symmetric(horizontal: 20),
